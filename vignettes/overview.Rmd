...
```{r, error = TRUE, purl = TRUE, highlight = TRUE}
...
```{r message = FALSE, warning = FALSE, results = "hide", error = TRUE, purl = TRUE}
...
```{r message = FALSE, error = TRUE, purl = TRUE, purl = TRUE}
...
```{r message = FALSE, warning = FALSE, error = TRUE, purl = TRUE}
MDG_shp <- getShp(ISO = "MDG", admin_level = "admin0")
MDG_shp_df <- as.MAPshp(MDG_shp)
MDG_PfPR2_10 <- getRaster(surface = "Plasmodium falciparum PR2-10", shp = MDG_shp, year = 2013)
MDG_PfPR2_10_df <- as.MAPraster(MDG_PfPR2_10)
