...
MMR_vivax_endem <- getRaster(surface = "Plasmodium vivax Endemicity", shp = MMR_shp)
MMR_vivax_endem_df <- as.MAPraster(MMR_vivax_endem)

p <- autoplot(MMR_vivax_endem_df, shp_df = MMR_shp_df, printed = FALSE)

vec <- getVecOcc(country = c("Myanmar"))
p[[1]] +
geom_point(data = vec, aes(longitude, latitude, col = species))+
  scale_shape_discrete(solid = TRUE, name = "Species")+
  scale_fill_distiller(name = "Vivax endemicity", palette = "RdYlBu")+
  ggtitle("Raw Vector Survey points\n + Plasmodium vivax Endemicity, 2010")
```
