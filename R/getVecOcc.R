################
##### To do:
##### - need to add rest of the species
##### - if '.malariaAtlasHidden' is not found use ---- devtools::load_all()
##### - Add a continent ID ( talk to Joe or Dan. M) - DONE
##### - Ask catherine about species calls
##### - edit Dan Ps functions such that they work wih both, example = listPoints, second message.
#####            --- what i have done so far is okay but would also need user to be able to select which they want as they ma use the funciton not within the getPR or getVecOcc
##### - can get rid of 'listOccur' function
################

# Download Vector Occurrences from the MAP database
...
# @examples
# #Download vector occurrence data for Nigeria and Cameroon and map the locations of these points using autoplot
# \donttest{
NGA_CMR_vec <- getVecOcc(country = c("Nigeria", "Cameroon"), species = "all")
# autoplot(NGA_CMR_vec)
#
...
#@seealso \code{autoplot} method for quick mapping of PR point locations (\code{\link{autoplot.pr.points}}).    ##### NEED TO CHECK THIS!!! ######
...
getVecOcc <- function(country = NULL,
  ...
  if (exists('available_countries_stored', envir =  .malariaAtlasHidden)){
    ...
    available_countries <- listPoints(printed = T, data = "vector points")
  }
  ...
  if(tolower(species) == "all") {
    columns <- 
      "&PROPERTYNAME=site_id,latitude,longitude,country,country_id,month_start,year_start,month_end,year_end,anopheline_id,species,id_method1,id_method2,sample_method1,sample_method2,sample_method3,sample_method4,assi,citation,geom,time_start,time_end"
  ...
  if("all" %in% tolower(c(country, ISO))) {
    ...
    if (any(c(!is.null(country),!is.null(ISO)))) {
      if (!(is.null(country))) {
        cql_filter <- "country"
        colname <-"country"
      } else if (!(is.null(ISO))) {
    ...
    class(df) <- c("vecocc.points", class(df))
